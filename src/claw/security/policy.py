"""Security policy for CLAW tool sandboxing.
...
- Per-agent sliding-window action rate limiting
- macOS-specific forbidden paths
"""
...
class SecurityPolicy:
    ...
    per_agent_trackers: dict[str, ActionTracker] = field(default_factory=dict)
    per_agent_max_per_hour: int = 100
    # Global tracker
    ...
    def record_action(self, agent_id: str | None = None) -> bool:
        """Record an action. Returns True if within rate limit."""
        global_count = self.tracker.record()
        if agent_id:
            if agent_id not in self.per_agent_trackers:
                self.per_agent_trackers[agent_id] = ActionTracker()
            agent_count = self.per_agent_trackers[agent_id].record()
            return (
                global_count <= self.max_actions_per_hour
                and agent_count <= self.per_agent_max_per_hour
            )
        return global_count <= self.max_actions_per_hour

    def is_rate_limited(self, agent_id: str | None = None) -> bool:
        if self.tracker.count() >= self.max_actions_per_hour:
            return True
        if agent_id and agent_id in self.per_agent_trackers:
            return self.per_agent_trackers[agent_id].count() >= self.per_agent_max_per_hour
        return False
