"""Pipeline metrics collector for CLAW orchestration.
...
All methods are synchronous — these are in-memory data structures.
"""
...
logger = logging.getLogger("claw.orchestrator.metrics")
...
class PipelineMetrics:
    ...
    Used by the orchestrator to wrap each agent execution call with timing
    and token tracking.
    """
    ...
    def start_run(self, task_id: str, run_number: int) -> PipelineRun:
    ...
    def start_agent(self, task_id: str, agent_name: str) -> AgentMetric:
    ...
    def get_runs(self, task_id: Optional[str] = None) -> list[PipelineRun]:
