...
class Dispatcher:
    ...
    async def route_task(self, task: Task, task_context: Optional[TaskContext] = None) -> str:
        ...
        # Build exclusion set from task's excluded_agents (populated by agent rotation)
        ...
        if excluded:
            ...
            # If ALL agents are excluded, clear and log warning (failsafe)
        ...
        # 7. Absolute fallback (respect exclusions)
    ...
    def _pick_random_agent(self, excluded: set[str] | None = None) -> str:
        ...
        if not candidates:
            candidates = self._available_agent_ids  # failsafe
    ...
    def _resolve_fallback(self, task_type: str, excluded: set[str] | None = None) -> str:
        ...
        # All excluded — fall back to absolute first available (ignore exclusions)
